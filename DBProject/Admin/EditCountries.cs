...
namespace DBProject.Admin
{
    public partial class EditCountries : Form
    {
        ...
        public EditCountries(int editId)
        {
            InitializeComponent();
            this.isEditing = true;
            this.editId = editId;
        ...
        private void checkEditing()
        {
            ...
            else
            {
                ...
                {
                    DataRow dr = db.QueryDataRow("SELECT * FROM Locations.Countries WHERE id = " + editId.ToString());
        ...
        private void addUpdateBtn_Click(object sender, EventArgs e)
        {
            if (nameInput.Text != "")
            {
                using (DBHelper db = new DBHelper())
                {
                    if (!isEditing)
                    {
                        if (db.SimpleQuery("INSERT INTO Locations.Countries (name) VALUES ('" + MiscHelpers.escapeSQL(nameInput.Text) + "')") >= 1)
                        {
                    ...
                    {
                        if (db.SimpleQuery("UPDATE Locations.Countries SET name = '" + MiscHelpers.escapeSQL(nameInput.Text) + "' WHERE id = " + editId) >= 1)
                        {
