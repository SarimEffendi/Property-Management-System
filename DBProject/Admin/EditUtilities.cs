...
{
    public partial class EditUtilities : Form
    {
        ...
        public EditUtilities(int editId)
        {
            InitializeComponent();
            this.isEditing = true;
            this.editId = editId;
        ...
        private void checkEditing()
        {
            ...
            else
            {
                ...
                {
                    DataRow dr = db.QueryDataRow("SELECT * FROM Property.Utilities WHERE id = " + editId.ToString());
            ...
            try
            {
                ...
                if (nameInput.Text != "")
                {
                    using (DBHelper db = new DBHelper())
                    {
                        if (!isEditing)
                        {
                            if (db.SimpleQuery("INSERT INTO Property.Utilities (name) VALUES ('" + nameInput.Text + "')") >= 1)
                            {
                        ...
                        {
                            if (db.SimpleQuery("UPDATE Property.Utilities SET name = '" + nameInput.Text + "' WHERE id = " + editId) >= 1)
                            {
